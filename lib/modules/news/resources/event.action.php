...
class UserAction extends BaseAction
{
	...
	function genActivation($email, $type, $return){
	...
	function activate(){
		...
		$code_query = $this->mySQL_r->prepare("SELECT `id`,`username`,CONCAT(`f_name`, ' ', `s_name`),`email` FROM `core_users` WHERE `act_c`=? AND FROM_UNIXTIME(?)<DATE_ADD(`act_t`,INTERVAL 7 DAY)");
		...
		$time = time();
		$code_query->bind_param('si', $code, $time);
	...
	function edit(){
		...
		if($id === NULL){
			return new ActionResult(
				...
				0,
				'Failed to edit user.<br />Error: <code>No user ID was provided</code>',
				B_T_FAIL
			);
		}
		...
		$s_groups	= (WebApp::post('s_group')===NULL)?		array()	:WebApp::post('s_group');
		...
		if($f_name=='' || $s_name=='' || $email==''){
			return new ActionResult(
				...
				'/admin/user/user_edit',
				0,
				'Failed to edit user.<br />Error: <code>Name/Email must not be empty</code>',
				B_T_FAIL
			);
		}
