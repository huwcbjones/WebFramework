...
\s*class NavBar extends Base
{
	...
	private function _fetchParentItems(){
		$item_query = $this->mySQL_r->prepare("SELECT `MID`, `PID`, `dropdown`,`divider`, `title`, `cat1`, `cat2`, `cat3` FROM `core_menu` INNER JOIN `core_pages` ON `PID`=`ID` WHERE `parent` IS NULL ORDER BY `MID` ASC");
		$item_query->execute();
	...
	private function _fetchChildItems($parents){
		...
		foreach($parents as $P_ID=>$parentItem){
			if($PID===NULL) $PID = -1;
			if(!$parentItem['DD'] || !$user->accessPage($PID)){
				break;
			}
...
\s*private function _getDropdown($item){
