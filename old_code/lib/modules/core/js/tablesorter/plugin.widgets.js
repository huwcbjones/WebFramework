...
(function($) {
  ...
  ts.themes = {
    "bootstrap": {
      ...
      caption:		'',
      header:		'',
      // give the header a gradient background
      footerRow:	'',
      footerCells:	'',
      icons:		'',
      // add "icon-white" to make them white; this icon class is added to the <i> in the header
	  sortNone   : 'glyphicon glyphicon-sort pull-right',
	  sortAsc    : 'glyphicon glyphicon-sort-by-attributes pull-right',
	  sortDesc   : 'glyphicon glyphicon-sort-by-attributes-alt pull-right',
      active: '',
      // applied when column is sorted
      hover: '',
      // use custom css here - bootstrap class may not override it
      filterRow: '',
      // filter row class
      even: '',
...
*/
  ts.storage = function(table, key, value, options) {
    table = $(table)[0];
    var cookieIndex, cookies, date, hasLocalStorage = false,
      values = {},
      c = table.config,
      $table = $(table),
  ...
  ts.addWidget({
    ...
    format: function(table, c, wo) {
      var time, classes, $header, $icon, $tfoot, themesAll = ts.themes,
        $table = c.$table,
        $headers = c.$headers,
        theme = c.theme || 'jui',
      ...
      if (!$table.hasClass('tablesorter-' + theme) || c.theme === theme || !table.hasInitialized) {
        ...
        .removeClass(c.theme === '' ? '' : 'tablesorter-' + c.theme).addClass('tablesorter-' + theme + ' ' + themes.table) // add theme widget class name
        .find('tfoot');
        if ($tfoot.length) {
          $tfoot.find('tr').addClass(themes.footerRow).children('th, td').addClass(themes.footerCells);
        }
        // update header classes
        $headers.addClass(themes.header).not('.sorter-false').bind('mouseenter.tsuitheme mouseleave.tsuitheme', function(event) {
    ...
    remove: function(table, c, wo) {
      ...
      .removeClass(themes.hover + ' ' + remove + ' ' + themes.active).find('.tablesorter-filter-row').removeClass(themes.filterRow);
  ...
  ts.addWidget({
    ...
    options: {
      ...
      // if true, server-side filtering should be performed because client-side filtering will be disabled, but the ui and events will still be used.
    ...
    remove: function(table, c, wo) {
      ...
      .unbind('addRows updateCell update updateRows updateComplete appendCache filterReset filterEnd search '.split(' ').join('.tsfilter ')).find('.tablesorter-filter-row').remove();
      for (tbodyIndex = 0; tbodyIndex < $tbodies.length; tbodyIndex++) {
        $tbody = ts.processTbody(table, $tbodies.eq(tbodyIndex), true); // remove tbody
  ...
  ts.filter = {
    ...
    regex: {
      ...
      // exact match (allow '==')
      nondigit: /[^\w,. \-()]/g,
      // replace non-digits (from digit & currency parser)
    ...
    types: {
      ...
      operators: function(filter, iFilter, exact, iExact, cached, index, table, wo, parsed) {
        if (/^[<>]=?/.test(iFilter)) {
          var cachedValue, result, c = table.config,
          ...
          cachedValue = (parsed[index] || parser.type === 'numeric') && !isNaN(query) && cached ? cached : isNaN(iExact) ? ts.formatFloat(iExact.replace(ts.filter.regex.nondigit, ''), table) : ts.formatFloat(iExact, table);
          if (/>/.test(iFilter)) {
            result = />=/.test(iFilter) ? cachedValue >= query : cachedValue > query;
          }
          if (/</.test(iFilter)) {
            result = /<=/.test(iFilter) ? cachedValue <= query : cachedValue < query;
          }
      ...
      range: function(filter, iFilter, exact, iExact, cached, index, table, wo, parsed) {
        if (/\s+(-|to)\s+/.test(iFilter)) {
          ...
          result = (parsed[index] || c.parsers[index].type === 'numeric') && !isNaN(range1) && !isNaN(range2) ? cached : isNaN(iExact) ? ts.formatFloat(iExact.replace(ts.filter.regex.nondigit, ''), table) : ts.formatFloat(iExact, table);
          if (range1 > range2) {
            tmp = range1;
            range1 = range2;
            range2 = tmp;
            ...
            if (wo.filter_functions[column] === true && !$header.hasClass('filter-false')) {
              ...
              for (string in wo.filter_functions[column]) {
                if (typeof string === 'string') {
                  options += options === '' ? '<option value="">' + ($header.data('placeholder') || $header.attr('data-placeholder') || '') + '</option>' : '';
    ...
    buildRow: function(table, c, wo) {
      ...
      for (column = 0; column < columns; column++) {
        ...
        $header = c.$headers.filter('[data-column="' + column + '"]:last');
        buildSelect = (wo.filter_functions && wo.filter_functions[column] && typeof wo.filter_functions[column] !== 'function') || $header.hasClass('filter-select');
        ...
        if (ts.getData) {
          ...
          disabled = (c.headers[column] && c.headers[column].hasOwnProperty('filter') && c.headers[column].filter === false) || $header.hasClass('filter-false');
        }
        if (buildSelect) {
    ...
    bindSearch: function(table, $el) {
      ...
      $el.unbind('keyup search filterReset').bind('keyup search', function(event, filter) {
        var $this = $(this);
        // emulate what webkit does.... escape clears the filter
        if (event.which === 27) {
          this.value = '';
          // liveSearch can contain a min value length; ignore arrow and meta keys, but allow backspace
        } else if ((typeof wo.filter_liveSearch === 'number' && this.value.length < wo.filter_liveSearch && this.value !== '') || (event.type === 'keyup' && ((event.which < 32 && event.which !== 8 && wo.filter_liveSearch === true && event.which !== 13) || (event.which >= 37 && event.which <= 40) || (event.which !== 13 && wo.filter_liveSearch === false)))) {
          return;
        }
    ...
    hideFilters: function(table, c) {
      ...
      c.$table.find('.tablesorter-filter-row').addClass('hideme').bind('mouseenter mouseleave', function(e) {
    ...
    findRows: function(table, filters, combinedFilters) {
      ...
      var cached, len, $rows, rowIndex, tbodyIndex, $tbody, $cells, columnIndex, childRow, childRowText, exact, iExact, iFilter, lastSearch, matches, result, searchFiltered, filterMatched, showRow, time, c = table.config,
        wo = c.widgetOptions,
        columns = c.columns,
        $tbodies = c.$tbodies,
        ...
        parsed = c.$headers.map(function(columnIndex) {
          return (ts.getData) ? ts.getData(c.$headers.filter('[data-column="' + columnIndex + '"]:last'), c.headers[columnIndex], 'filter') === 'parsed' : $(this).hasClass('filter-parsed');
        }).get();
      if (c.debug) {
        time = new Date();
      }
      for (tbodyIndex = 0; tbodyIndex < $tbodies.length; tbodyIndex++) {
        if ($tbodies.eq(tbodyIndex).hasClass(ts.css.info)) {
          continue;
        ...
        if (combinedFilters === '' || wo.filter_serversideFiltering) {
          ...
          if (searchFiltered && $rows.filter(':visible').length === 0) {
          ...
          for (rowIndex = 0; rowIndex < len; rowIndex++) {
            ...
            if (ts.filter.regex.child.test(childRow) || (searchFiltered && ts.filter.regex.filtered.test(childRow))) {
              continue;
            }
            showRow = true;
              ...
              if (filters[columnIndex] || wo.filter_anyMatch) {
                ...
                if (wo.filter_functions && wo.filter_functions[columnIndex]) {
                  if (wo.filter_functions[columnIndex] === true) {
                    ...
                    result = (c.$headers.filter('[data-column="' + columnIndex + '"]:last').hasClass('filter-match')) ? iExact.search(iFilter) >= 0 : filters[columnIndex] === exact;
    ...
    buildSelect: function(table, column, updating, onlyavail) {
      ...
      var indx, rowIndex, tbodyIndex, len, currentValue, txt, c = table.config,
        wo = c.widgetOptions,
        $tbodies = c.$tbodies,
        arry = [],
      ...
      for (tbodyIndex = 0; tbodyIndex < $tbodies.length; tbodyIndex++) {
        ...
        for (rowIndex = 0; rowIndex < len; rowIndex++) {
          ...
          if (onlyavail && c.cache[tbodyIndex].row[rowIndex][0].className.match(wo.filter_filteredRow)) {
            continue;
          }
      ...
      arry = (ts.sortNatural) ? arry.sort(function(a, b) {
      ...
      for (indx = 0; indx < arry.length; indx++) {
        ...
        options += arry[indx] !== '' ? '<option value="' + txt + '"' + (currentValue === txt ? ' selected="selected"' : '') + '>' + arry[indx] + '</option>' : '';
      }
      c.$table.find('thead').find('select.tablesorter-filter[data-column="' + column + '"]')[updating ? 'html' : 'append'](options);
    },
    buildDefault: function(table, updating) {
      var columnIndex, $header, c = table.config,
        wo = c.widgetOptions,
        columns = c.columns;
      ...
      for (columnIndex = 0; columnIndex < columns; columnIndex++) {
        ...
        if (($header.hasClass('filter-select') || wo.filter_functions && wo.filter_functions[columnIndex] === true) && !$header.hasClass('filter-false')) {
          if (!wo.filter_functions) {
            wo.filter_functions = {};
          }
  ...
  ts.addWidget({
    id: "stickyHeaders",
    priority: 60,
    // sticky widget must be initialized after the filter widget!
    options: {
      stickyHeaders: '',
    ...
    format: function(table, c, wo) {
      ...
      var $cell, $table = c.$table,
        ...
        $stickyTable = wo.$sticky = $table.clone().addClass('containsStickyHeaders').css({
        ...
        resizeHeader = function() {
          ...
          $stickyTable.css({
            left: $attach.length ? parseInt($attach.css('padding-left'), 10) + parseInt($attach.css('margin-left'), 10) + parseInt($table.css('border-left-width'), 10) : $thead.offset().left - $win.scrollLeft() - spacing,
            width: $table.width()
          });
      ...
      $table.addClass('hasStickyHeaders').bind('sortEnd.tsSticky', function() {
        $header.filter(':visible').each(function(indx) {
          $cell = $stickyCells.filter(':visible').eq(indx).attr('class', $(this).attr('class'))
          // remove processing icon
          .removeClass(ts.css.processing + ' ' + c.cssProcessing);
          if (c.cssIcon) {
            $cell.find('.' + ts.css.icon).attr('class', $(this).find('.' + ts.css.icon).attr('class'));
          }
        });
      ...
      $header.find(c.selectorSort).add(c.$headers.filter(c.selectorSort)).each(function(indx) {
        ...
        if (c.cancelSelection) {
          $cell.attr('unselectable', 'on').bind('selectstart', false).css({
            'user-select': 'none',
            'MozUserSelect': 'none'
          });
        }
      ...
      $win.bind('scroll.tsSticky resize.tsSticky', function(event) {
        ...
        $stickyTable.removeClass(prefix + 'visible ' + prefix + 'hidden').addClass(prefix + isVisible).css(cssSettings);
    ...
    remove: function(table, c, wo) {
      c.$table.removeClass('hasStickyHeaders').unbind('sortEnd.tsSticky pagerComplete.tsSticky').find('.tablesorter-stickyHeader').remove();
  ...
  ts.addWidget({
    ...
    format: function(table, c, wo) {
      ...
      var $rows, $columns, $column, column, storedSizes = {},
        $table = c.$table,
        mouseXPosition = 0,
        $target = null,
        $next = null,
      ...
      $columns.each(function() {
        ...
        $column.find('.tablesorter-wrapper').append('<div class="tablesorter-resizer" style="cursor:w-resize;position:absolute;z-index:1;right:-' + padding + 'px;top:0;height:100%;width:20px;"></div>');
        ...
        stopResize();
      }).find('.tablesorter-resizer,.tablesorter-resizer-grip').bind('mousedown', function(event) {
      ...
      $table.find('thead:first').bind('mouseup.tsresize mouseleave.tsresize', function() {
        stopResize();
      })
  ...
  ts.resizableReset = function(table) {
    ...
    if (ts.storage) {
      ts.storage(table, 'tablesorter-resizable', {});
    }
  };
