 ...
 * News Resource Class
 *
 * @category   Module.News.Resource
 * @package    news/resource.php
...
class NewsResource extends BaseResource
{
	...
	function __construct($parent, $mode){
		$this->parent = $parent;
		$this->mySQL_r = $parent->mySQL_r;
		
		if($mode=='article'){
			require_once dirname(__FILE__) . '/resources/article.resource.php';
			return new ArticleResource($parent);
		}elseif($mode=='event'){
			require_once dirname(__FILE__) . '/resources/event.resource.php';
			return new EventResource($parent);
		}else{
